...
## ⭐ Session 20/07/2026 (soir, 3e lot) — Blog admin thème clair + carrousel cadrage ✅
- **Blog admin — texte invisible en thème CLAIR** (`pages/admin/AdminCitadelleBlog.js`) : le composant était codé pour thème sombre (couleurs `rgba(255,255,255,...)`). Branché `useAdminTheme()` (`/context/AdminThemeContext.js` fournit `currentTheme.text/textSecondary/textMuted/bgSection/border`). Corrigés : (1) Top 10 — numéros >3 et compteur de vues → `currentTheme.textMuted/textSecondary` ; (2) pagination — texte « Page x/y » → `currentTheme.textSecondary`, fond des boutons → `currentTheme.bgSection`+bordure. 
- **Modal stats d'article — fond FIXE sombre `#0F2039`** : les textes hérités (titre article, valeurs KPI Impressions/Visiteurs/Vues, valeurs appareils desktop/mobile, icône X) prenaient la couleur du thème → NOIR sur navy en thème clair = invisibles. Forcés en clair `#F1F5F9` (et X en `rgba(255,255,255,0.75)`). Les labels rgba blancs restent (fond toujours sombre). Vérifié en thème clair par screenshots (Top10, modal).
- **Carrousel accueil — image rognée disgracieuse** (`components/citadelle/ListingsCarousel.jsx`, ligne img) : `object-cover` rognait les captures de sites par le centre. Ajout `object-top` → cadrage par le haut (en-tête/hero du site), convention marketplace. Vérifié en prod (Site WordPress / e-commerce badges bien cadrés).
...
## ⭐ Session 22/07/2026 — Prix réserve masqué + Estimation acheteurs + Modal choix estimation ✅
- **Prix de réserve masqué** (`CitadelleListingDetail.js` ~ligne 408) : l'affichage du montant (`listing.price`) remplacé par la mention neutre « Prix de réserve confidentiel » (toujours, quand `auction_show_reserve`). Reste une checkbox « Afficher le prix de réserve » dans le formulaire de création (sans effet sur le montant désormais) — NON retirée (proposé au client, en attente).
- **Estimation Standard côté acheteurs** (`CitadelleServices.js` ligne 244) : bug d'espace en base — en PROD le service s'appelle `'Estimation Standard '` (espace final) alors que le filtre comparait `'Estimation Standard'`. Fix : `.includes((s.title||'').trim())`. Vérifié en prod : Estimation Standard apparaît désormais dans « Pour les acheteurs ». ⚠️ Même piège pour `SellerServicesUpsell` (find `s.title === t.title`) — mais résolu côté modal car les 2 estimations viennent du même filtre ; à surveiller si re-seed. Commande de nettoyage DB fournie (optionnelle) : `citadelle_services.updateOne({title:'Estimation Standard '},{$set:{title:'Estimation Standard'}})`.
- **Encart estimation annonce → modal de choix** (`components/citadelle/SellerServicesUpsell.jsx` + `CitadelleListingDetail.js`) : ajout prop `chooser`. En mode chooser : un seul bouton « Faire estimer ce site » SANS prix (évite confusion valeur du site vs tarif service), qui ouvre un modal listant Estimation Standard/Expert avec prix (contexte clair). `data-testid`: `upsell-open-chooser`, `estimation-chooser-modal`, `estimation-chooser-close`, `estimation-choose-{id}`. L'usage vendeur (sans `chooser`) est inchangé.
- Déployé (git pull commit f767036 + yarn build + reload nginx) et **vérifié en production** (page Services + encart annonce). Backend inchangé.
...
- **Cartes carrousel accueil (dark) — ajustées à la demande client** : zone texte **fond blanc**, **texte titre en bleu** (#0F2747), prix en or, **bordures dorées épaisses** (3px tout autour + filet doré 3px sous l'image), **nombre de vues supprimé**. La variante `light` (page annonces) reste inchangée (bordure subtile), vues aussi retirées.
- **Page annonces (`CitadelleListings.js`) — ajustements client** : bandeau vendeur « Votre actif numérique mérite la Citadelle » déplacé **AU-DESSUS** du carrousel « À la Une ». Carrousel variante `light` alignée sur l'accueil : **bordures dorées épaisses 3px** + filet doré sous l'image ; placeholder sans image = **logo Citadelle sur fond bleu** (logo dans pastille blanche pour lisibilité). Vérifié par screenshot.
- **Bannière preuve sociale « vente récente »** (`components/citadelle/RecentSaleBanner.jsx` + `GET /listings/recent-sales`) : affichée en haut de l'**espace membre** (routes `/citadelle/espace-membre`), **une fois par session** (sessionStorage `citadelle_sale_banner_seen`), pendant **60 s** puis disparaît (ou fermeture manuelle). Message « Nouvelle vente sécurisée par La Garde ! « {titre} » — {type} vendu {prix} € via La Citadelle », pastille « En direct » pulsée, dégradé navy→or. Vente tirée au hasard parmi les 5 plus récentes (variété). Intégré dans `CitadelleLayout` après `ModerationBanner`. Endpoint testé (5 ventes) + screenshot bannière.
- **Newsletter hebdo — 3 fenêtres d'articles en bas** (`services/email_service/citadelle/newsletter.py` + `services/newsletter_scheduler.py` + `routes/citadelle/newsletter.py`) : bloc « À lire cette semaine » ajouté après le CTA annonces, avec 3 fenêtres — **Derniers articles** (hors guides/chroniques), **Guides de La Citadelle** (`guide-la-citadelle`), **Chroniques de La Garde** (`chroniques-la-garde`). Helper `fetch_blog_sections(since_iso, per_section=3)` : en envoi hebdo réel filtre sur les articles publiés dans la période (`published_at >= now - days_back`), en aperçu/force prend les plus récents. `build_newsletter_html` / `send_newsletter_digest_email` reçoivent `blog_sections` ; snapshot stocké dans l'historique (`blog_sections_snapshot`) + previews live/historique mis à jour. Liens articles traqués (`/newsletter/click`). Vérifié via aperçu admin (3 fenêtres + liens) + screenshot du rendu. Aucun email envoyé.
...
- **Même upsell sur la fiche détail** (`CitadelleListingDetail.js`, propriétaire, `ownerUpsellStep`) : sous l'encart « Mettre à la Une », bouton **Non merci** (`owner-boost-decline`) → `SellerServicesUpsell` en **mode empilé** (`stacked`, cartes en ligne pleine largeur adaptées à la colonne étroite).
- **Upsell estimations ACHETEUR** (`CitadelleListingDetail.js`) : composant `SellerServicesUpsell` généralisé (props `targetServices`, `heading`, `subheading`, `clientMessage`, `onRequireAuth`, `testid`) + constantes exportées `SELLER_SERVICES` / `BUYER_ESTIMATION_SERVICES`. Pour tout non-propriétaire (annonce non vendue), bloc **« Ce site vaut-il le coup ? »** (`buyer-estimation-upsell`) proposant **Estimation Standard 49 €** + **Estimation Expert 149 €** → `POST /payments/service/checkout` avec message contextuel (réf annonce). Non connecté → clic « Acheter » ouvre la modale d'auth (`onRequireAuth`). Vérifié cURL (checkout 200 + URL Stripe) + screenshot.
- **Page Services — estimations aussi côté acheteur** (`CitadelleServices.js`) : les 2 estimations (`target_category=vendeur`) sont désormais affichées AUSSI dans la zone « Pour les acheteurs » (en tête, avant Audit SEO/Sécurité) via ajout frontend à `buyerServices` (DRY, elles restent visibles côté vendeurs). Vérifié par screenshot.
- **Pop-up estimation à l'entame d'une conversation** (`CitadelleListingDetail.js` + `backend/routes/citadelle/messages.py`) : après l'envoi du message de contact au vendeur, affichage d'une pop-up (`contact-estimation-popup`) réutilisant `SellerServicesUpsell` (heading « Votre message est bien parti ! » + accroche vente) avec **Estimation Standard 49 €** + **Estimation Expert 149 €** et bouton **Non merci** (`upsell-decline`) qui redirige vers la conversation (`pendingConversationId`). La pop-up ne s'affiche QU'À la **première prise de contact** : `POST /messages/send` renvoie désormais `created` (bool, nouvelle conversation) — si `false`, redirection directe sans pop-up. Vérifié cURL (send #1 → created True, send #2 → created False) + screenshot pop-up. ⚠️ Éviter de déclencher l'envoi réel en test (notification email vendeur — Règle 6).
...
## 🐛 Session 14/08/2026 — Responsive + Mobile-Friendly PWA "Papa en Mousse"
- **Problème** : la PWA admin "Papa en Mousse" (`/admin-live`) affichait le bandeau d'alertes globales du site (ex : "Webinaire le 15 mars") et avait des cibles tactiles trop petites sur mobile.
- **Fix 1 — GlobalAlerts.js** : Pattern wrapper `GlobalAlertsInner` + `GlobalAlerts` (règle des Hooks respectée). Le composant principal vérifie `useLocation().pathname.startsWith("/admin-live")` → `return null` si vrai. La bannière n'apparaît plus dans la PWA.
