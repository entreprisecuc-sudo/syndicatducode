...
- **Pop-up estimation à l'entame d'une conversation** (`CitadelleListingDetail.js`) : après l'envoi du message de contact au vendeur (au lieu de rediriger immédiatement), affichage d'une pop-up (`contact-estimation-popup`) réutilisant `SellerServicesUpsell` (heading « Votre message est bien parti ! » + accroche vente) avec **Estimation Standard 49 €** + **Estimation Expert 149 €** et bouton **Non merci** (`upsell-decline`) qui redirige alors vers la conversation (`pendingConversationId`). Vérifié par screenshot (2 cartes + accroche + Non merci). ⚠️ Éviter de déclencher l'envoi réel de message en test (notification email vendeur — Règle 6).
