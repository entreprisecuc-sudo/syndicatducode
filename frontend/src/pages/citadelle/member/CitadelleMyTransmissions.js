...
const ORDER_STATUS = {
  en_attente: { color: "#F59E0B", bg: "rgba(245,158,11,0.1)", Icon: Clock },
  en_cours:   { color: "#3B82F6", bg: "rgba(59,130,246,0.1)", Icon: AlertCircle },
  termine:    { color: "#22C55E", bg: "rgba(34,197,94,0.1)",  Icon: CheckCircle },
  annule:     { color: "#DC2626", bg: "rgba(220,38,38,0.1)",  Icon: Ban },
};

const formatDate = (isoStr, lang = "fr") => {
  if (!isoStr) return "—";
  return new Date(isoStr).toLocaleDateString(lang === "en" ? "en-GB" : "fr-FR", { day: "2-digit", month: "short", year: "numeric" });
};
    ...
    <CitadelleLayout>
      <div className="max-w-3xl mx-auto px-4 md:px-6 py-10" data-testid="my-transmissions">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-2xl font-black" style={{ color: C.blue }}>{t("member.trans_title")}</h1>
            <p className="text-sm" style={{ color: C.textMuted }}>{t("member.trans_subtitle")}</p>
          </div>
        </div>

        {/* ── Section 1 : Transmissions ─────────────────────────────── */}
        <section className="mb-12">
          <h2 className="text-base font-bold mb-4" style={{ color: C.blue }}>{t("member.trans_section_transmissions")}</h2>
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-2xl border p-10 text-center" style={{ borderColor: C.border }} data-testid="my-transmissions-empty">
              ...
              <p className="text-sm" style={{ color: C.textMuted }}>{t("member.trans_empty")}</p>
            </div>
          ) : (
            <div className="space-y-3">
              {items.map((item) => (
                <div key={item.id} className="bg-white rounded-2xl border p-5 flex items-center gap-4" style={{ borderColor: C.border }} data-testid={`transmission-item-${item.id}`}>
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="font-bold text-sm truncate" style={{ color: C.blue }}>{item.asset_title}</p>
                    <p className="text-xs" style={{ color: C.textMuted }}>
                      {item.role === "buyer" ? t("member.trans_doc_buyer") : t("member.trans_doc_seller")} · {t("member.trans_dossier")} {item.dossier_number}
                    </p>
                  </div>
                  <button onClick={() => download(item)} disabled={busy === item.id} data-testid={`download-transmission-${item.id}`}
                    ...
                    {busy === item.id ? <Loader2 size={15} className="animate-spin" /> : <Download size={15} />} PDF
                  </button>
                </div>
              ))}
            </div>
        ...
        {/* ── Section 2 : Commandes ─────────────────────────────────── */}
        <section>
          <h2 className="text-base font-bold mb-4" style={{ color: C.blue }}>{t("member.trans_section_orders")}</h2>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-1">
                          ...
                          <span className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-full font-semibold whitespace-nowrap"
                            ...
                            <StatusIcon size={10} />
                            {t(`member.order_status.${ORDER_STATUS[order.status] ? order.status : "en_attente"}`)}
                          </span>
                        </div>
                        <p className="text-xs" style={{ color: C.textMuted }}>
                          {t("member.trans_ordered_on")} {formatDate(order.created_at, i18n.language)} · {t("member.trans_ref")} {order.id.slice(0, 8).toUpperCase()}
                        </p>
                      </div>
                      <p className="font-black text-base shrink-0" style={{ color: C.gold }}>
                        {order.amount?.toLocaleString(i18n.language === "en" ? "en-GB" : "fr-FR")} €
                      </p>
                    </div>
                    {order.admin_note && (
                      <p className="text-xs mt-2 pl-3 border-l-2 italic"
                        style={{ color: C.textMuted, borderColor: C.gold + "55" }}>
                        {t("member.trans_note")} {order.admin_note}
                      </p>
                    )}
                  </div>
                );
